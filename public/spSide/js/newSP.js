$(document).ready(function(){
  ...
  var providerStore = sessionStorage.getItem('providerArray');

  if (!providerStore) { // check if an item is already registered
    providerArray = [];
  } else {
    providerArray = JSON.parse(providerStore);
     // else parse whatever is in
  }
  ...
  $('#sp_signUP').click(function(){
    ...
    function sp (uName, sName, sPassword, sEmail){
      ...
      sessionStorage.setItem('providerArray', JSON.stringify(providerArray));
